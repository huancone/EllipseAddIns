...
using MSO627BombasExcelAddIn.Properties;
...
namespace MSO627BombasExcelAddIn
{
    public partial class RibbonEllipse
    {
        ...
        private void FormatSheet()
        {
            ...
            listaEquipos = new[] { "WPHV-VNM-10", "WPHH-DS-10", "WPHV-GNVN-8", "WPHS-SS-6", "WPHS-SS-10", "WPF-2201-4", "WPF-2250-6", "WPF-2250-10", "WPF-2400-6", "WPF-3201-6", "WPF-3230-8" }
        ...
        private void CargarDatosBombas()
        {
            ...
            while (!string.IsNullOrEmpty("" + _cells.GetCell(1, currentRow).Value))
            {
                try
                {
                    ...
                    foreach (var item in listaUbicaciones.Where(item => item.Nombre == _cells.GetEmptyIfNull(_cells.GetCell(2, currentRow).Value)))
                    {
                        origen = item.Nombre + "/" + item.X + "/" + item.Y + "/" + item.Z;
                    }

                    foreach (var item in listaUbicaciones.Where(item => item.Nombre == _cells.GetEmptyIfNull(_cells.GetCell(5, currentRow).Value)))
                    {
                ...
                catch (Exception ex)
                {
                    ...
                    Debugger.LogError("RibbonEllipse.cs:CargarDatosBombas()", ex.Message, _eFunctions.DebugErrors);
                }
                finally
                {
                    _cells.GetCell(ResultColumn01, currentRow).Select();
        ...
        private List<Ubicaciones> ImportFile()
        {
            ...
            foreach (var p in archivoUbicaciones)
            {
                try
                {
                    listaUbicaciones.Add(p);
                }
                catch (Exception ex)
                {
                    Debugger.LogError("RibbonEllipse.cs:ImportFile()", ex.Message, _eFunctions.DebugErrors);
                }
            }
