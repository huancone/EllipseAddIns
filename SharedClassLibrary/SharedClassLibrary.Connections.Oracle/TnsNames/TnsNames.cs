...
namespace erl.Oracle.TnsNames
{
    ...
    public static class TnsNames
    {
        ...
        public static TnsNamesFile OpenTnsNamesFile([NotNull] string filePath)
        {
            var tnsNamesFileInfo = new TnsNamesFileInfo(filePath, TnsNamesSource.CustomPath);

            return OpenTnsNamesFile(tnsNamesFileInfo);
        }
        ...
        public static TnsNameInfo[] OpenTnsNameInfo(string filePath)
        {
            ...
            return tnsNameInfoList.ToArray();
        }

        public static TnsNameInfo[] OpenTnsNameInfo(TnsNamesSource source)
        {
            var tnsNameInfoList = new List<TnsNameInfo>();
            switch (source)
            {
                case TnsNamesSource.TnsAdminEnvironmentVariable:
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(tnsAdminEnv) && FileExists(AppendPath(tnsAdminEnv, @"\tnsnames.ora")))
                    {
                        var tnsNamesFileInfo = new TnsNamesFileInfo(AppendPath(tnsAdminEnv, @"\tnsnames.ora"), TnsNamesSource.TnsAdminEnvironmentVariable);
                        var tnsNameFile =  OpenTnsNamesFile(tnsNamesFileInfo);
                        if(tnsNameFile != null) tnsNameInfoList.AddRange(tnsNameFile.TnsNames);
                    }
                ...
                case TnsNamesSource.OracleHomeEnvironmentVariable:
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(oracleHomeEnv) && FileExists(AppendPath(oracleHomeEnv, @"\network\admin\tnsnames.ora")))
                    {
                        var tnsNamesFileInfo = new TnsNamesFileInfo(AppendPath(oracleHomeEnv, @"\network\admin\tnsnames.ora"), TnsNamesSource.OracleHomeEnvironmentVariable);
                        var tnsNameFile = OpenTnsNamesFile(tnsNamesFileInfo);
                        if (tnsNameFile != null) tnsNameInfoList.AddRange(tnsNameFile.TnsNames);
                    }
                ...
                case TnsNamesSource.PathEnvironmentVariable:
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(pathEnv))
                    {
                        ...
                        foreach (var tnsNameFile in result.Select(OpenTnsNamesFile).Where(tnsNameFile => tnsNameFile != null))
                            tnsNameInfoList.AddRange(tnsNameFile.TnsNames);
                    }
                    break;
                }
                default:
                    throw new ArgumentException("Not a valid source type", nameof(source));
            }
        ...
        public static string GetSourceName(TnsNamesSource source)
        {
            switch (source)
            {
                ...
                case TnsNamesSource.IfileEntry:
                    return "IFile";
                case TnsNamesSource.CustomPath:
                    return "ORA FILE URL";
                default:
